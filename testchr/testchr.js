...
let playground = {
	name:'playground',
	code: wrapModule(playgroundCHRCode),
...
// 	name:'taskflow',
// 	code: 
// `CHR.Modules.Taskflow = {
// 	${versionCode}
// 	taskflow: ` + wrap(taskflowFunction) + `
// };
// `,
// 	testcode: wrapExpr(taskflowCode)
// };


// let taskflowCode = function() {
//     chr = new CHR();
//     let init = Taskflow.init(chr,{},'Taskflow');

//     // reverse order
// 	init.module.next('start');
// 	init.module.init();
// 	chr.resolve();
...
// let taskflowFunction = 	function() {

// 	let mkUnique = (function() { let uniq = 0; return function() { return uniq++; } })();

// 	let clear = function(clazz) {
// 		if (!clazz)
// 			return;
// 		let outputs = document.querySelectorAll('.' + clazz);
...
// 	let removeOutput = function(clazz) {
// 		if (!clazz)
// 			return;
// 		let outputs = document.querySelectorAll('.' + clazz);
// 		outputs.forEach(remove);
// 	};

// 	let putOutput = function(html,clazz = 'outputblock') {
// 		let output = document.getElementById('outputContainer');
// 		html.setAttribute('class',clazz);
// 		output.appendChild(html);
// 	};

// 	let makeActionElem = function(id,clazz) {
// 		let div = document.createElement('div');
// 		if (id)
// 			div.setAttribute('id',id);
// 		div.setAttribute('class','actionelem outputblock');
// 		return div;
// 	};

// 	let makeDiv = function(clazz) {
// 		let div = document.createElement('div');
// 		if (clazz)
...
// 	let addText = function(html,l,clazz) {
// 		if (html.constructor === String)
// 			html = document.getElementById(html);
// 		let div = makeDiv(clazz);
// 		div.appendChild(document.createTextNode(l));
// 		html.appendChild(div);
// 		return div;
// 	};

// 	let remove = html => { html.remove(); };
// 	let onclickRemove = r => { r.onclick = function() { remove(r); }}

// 	let setProp = (html,p,v) => { html[p] = v }; 

// 	// let setAction = (e,f) => { f(e) };

// 	let empty = v => v.trim().length === 0;
// 	let moreThan = ref => (v => v.trim() > ref);
// 	let lessThan = ref => (v => v.trim() < ref);
// 	let equal = ref => (v => v === ref);
// 	let notEqual = ref => (v => v !== ref);
// 	let emptyArray = v => (!Array.isArray(v) || v.length === 0);

// 	- init >>> 
// 		label('introduction','Introduction','en'),
// 		label('userData','User information','en'),
// 		label('name','Name','en'),
// 		label('age','Age','en'),
// 		label('gender','Gender','en'),
// 		label('male','Male','en'),
// 		label('female','Female','en'),
// 		label('favorites','Favorite things', 'en'),
// 		label('someof','Some of','en'),
// 		label('dogs','Dogs','en'),
// 		label('cats','Cats','en'),
// 		label('computers','Computers','en'),
// 		label('pianos','Pianos\'s','en'),
// 		label('end','End...','en'),

// 		text('introduction','This is an introduction...','en'),
// 		text('introduction','(Click to continue)','en'),
// 		text('userData','Please provide the following information','en'),
// 		text('favorites','Please select some of your favorite things','en'),
// 		text('end','Thank you for participating!','en'),
// 		text('end','These are your results!','en'),
// 		text('end','(Click to close)','en'),

// 		props('userData',['name','age','gender']),
// 		props('favorites',['someof']),

// 		type('name','string'),
// 		type('age','integer'),
// 		type('gender',{oneof:['male','female']}),
// 		type('someof',{'manyof':['dogs','cats','computers','pianos']}),

// 		action('introduction','click'),
// 		action('userData','button'),
// 		action('favorites','button'),
// 		action('end','click'),

// 		next('introduction','userData'),
// 		next('userData','favorites'),
// 		next('favorites','end'),

// 		failOn('userData','name',empty,'Name should not be empty','notification taskId error'),
// 		failOn('userData','age',empty,'Age should not be empty','notification taskId error'),
// 		failOn('userData','age',lessThan(10),'Age should be more than 10','notification taskId error'),
// 		failOn('userData','gender',equal('null'),'Gender should be filled in','notification taskId error'),
// 		failOn('favorites','someof',emptyArray,'Select at least one favorite thing...','notification taskId error')
// 		;

// 	result(taskId,'ok') >>> validResult(taskId);

// 	- next('start') >>> next('introduction');

// 	completed(taskId,u), next(taskId,next) / next(taskId)  >>> next(next);

// 	next(taskId) >>> (u = mkUnique(), html = makeActionElem(taskId + '-' + u), view(taskId,u,html), putOutput(html,'task ' + taskId));

// 	view(taskId,u,html), label(taskId,l,_) >>> addText(html,l,'label');
// 	view(taskId,u,html), text(taskId,l,_) >>> addText(html,l,'text');
// 	view(taskId,u,html), props(taskId,ps) >>> addProp(taskId,u,html,ps);
// 	view('end',u,html) >>> addValue('end',u,html,['name','age','gender','someof']);
// 	view(taskId,u,html), action(taskId,act) >>> setAction(taskId,u,html,act);


// 	let forEach = function(arr,f) { arr.forEach(f); };

// 	- addProp(taskId,u,html,ps) | Array.isArray(ps) >>> forEach(ps, p => { module.addProp(taskId,u,html,p); chr.resolve(); });
// 	- addProp(taskId,u,html,p), label(p,L,_), type(p,T) >>> addProp(taskId,u,p,html,L,T);


// 	let makeLabelInput = function(lab,input,clazz) {
// 		let p = makeDiv();
...
// 	let appendChild = (node,child) => { node.appendChild(child); };
// 	let collectValue = i => (() => { return i.value; });
// 	let collectRadio = (name,def) => (() => { 
// 		let s = document.querySelector('input[name="' + name + '"]:checked');
// 		if (s) 
// 			return s.value;
// 		else
// 			return def;
...
// 	let collectChecks = (name,def) => (() => { 
// 		let cs = document.querySelectorAll('input[name="' + name + '"]:checked');
// 		if (cs) 
// 			return [...cs.values()].map(c => c.value);
// 		else
// 			return def;
// 	});

// 	let createInput = (type,lab) => {
// 		let i = document.createElement('input');
// 		i.setAttribute('type',type);
// 		let p = makeLabelInput(lab,i,'prop');
// 		return {i,p};
// 	}

// 	- addProp(taskId,u,prop,html,lab,'string') >>> (
// 		pi = createInput('text',lab),
// 		appendChild(html,pi.p),
// 		collect(taskId,u,prop,collectValue(pi.i))
// 		);

// 	- addProp(taskId,u,prop,html,lab,'integer') >>> (
// 		pi = createInput('number',lab),
// 		appendChild(html,pi.p),
// 		collect(taskId,u,prop,collectValue(pi.i))
// 		);

// 	- addProp(taskId,u,prop,html,lab,{oneof:radios}) | Array.isArray(radios) >>> (
// 		r = makeDiv('radio ' + taskId),
// 		u2 = mkUnique(),
// 		name = taskId+'-'+u2,
// 		forEach(radios, p => { module.addRadio(r,name,p); chr.resolve(); }),
// 		rs = makeLabelInput(lab,r,'prop'),
// 		appendChild(html,rs),
// 		collect(taskId,u,prop,collectRadio(name,'null'))
// 		);

// 	- addRadio(html,name,value), label(value,lab,_) >>> (
// 		i = document.createElement('input'),
// 		i.setAttribute('type','radio'),
// 		i.setAttribute('name',name),
// 		i.setAttribute('value',value),
// 		p = makeLabelInput(lab,i),
// 		appendChild(html,p)
// 	);


// 	- addProp(taskId,u,prop,html,lab,{manyof:checks}) | Array.isArray(checks) >>> (
// 		r = makeDiv('checks ' + taskId),
// 		u2 = mkUnique(),
// 		name = taskId+'-'+u2,
// 		forEach(checks, p => { module.addCheck(r,name,p); chr.resolve(); }),
// 		rs = makeLabelInput(lab,r,'prop'),
// 		appendChild(html,rs),
// 		collect(taskId,u,prop,collectChecks(name,[]))
// 		);


// 	- addCheck(html,name,value), label(value,lab,_) >>> (
// 		i = document.createElement('input'),
...
// 	- addValue(taskId,u,html,ps) | Array.isArray(ps) >>> forEach(ps, p => { module.addValue(taskId,u,html,p); chr.resolve(); });
// 	- addValue(taskId,u,html,p), label(p,L,_), type(p,T) >>> addValue(taskId,u,p,html,L,T);


// 	- addValue(taskId,u,prop,html,lab,'string'), value(taskId,uAny,prop,v) >>> (
...
// 	- addValue(taskId,u,prop,html,lab,'integer'), value(taskId,uAny,prop,v) >>> (
// 		i = document.createElement('span'),
// 		i.appendChild(document.createTextNode((v))),
// 		p = makeLabelInput(lab,i),
// 		appendChild(html,i)
...
// 	// - addValue(taskId,u,prop,html,lab,{oneof:radios}) | Array.isArray(radios) >>> (
// 	// 	div = makeDiv('radioOutput ' + taskId),
// 	// 	forEach(radios, p => { module.addRadioValue(taskId,u,p,div); chr.resolve(); }),
// 	// 	rs = makeLabelInput(lab,div,'prop'),
// 	// 	appendChild(html,rs)
// 	// 	);

// 	// - addRadioValue(taskId,u,prop,html), label(prop,lab,_), value(taskId,uAny,prop,v) >>> (
// 	// 	i = document.createElement('span'),
// 	// 	i.appendChild(document.createTextNode((v))),
// 	// 	p = makeLabelInput(lab,i),
// 	// 	appendChild(html,p)
...
// 	// - addValue(taskId,u,prop,html,lab,{manyof:checks}) | Array.isArray(checks) >>> (
// 	// 	div = makeDiv('checksOutput ' + taskId),
// 	// 	forEach(checks, p => { module.addRadioValue(taskId,u,p,div); chr.resolve(); }),
// 	// 	rs = makeLabelInput(lab,div,'prop'),
// 	// 	appendChild(html,rs)
// 	// 	);


// 	// - addCheckValue(html,name,value), label(value,lab,_) >>> (
// 	// 	i = document.createElement('input'),
// 	// 	i.setAttribute('type','checkbox'),
// 	// 	i.setAttribute('name',name),
// 	// 	i.setAttribute('value',value),
...
// 	- setAction(taskId,u,html,'click') >>> setProp(html,'onclick', function() { module.acted(taskId,u); chr.resolve(); });

// 	- setAction(taskId,u,html,'button') >>> (
// 		div = makeDiv('action'),
// 		b = document.createElement('button'),
// 		b.appendChild(document.createTextNode('OK')),
// 		setProp(b,'onclick', function() { module.acted(taskId,u); chr.resolve(); }),
// 		div.appendChild(b),
...
// 	let apply = function(f) { return f(); }
// 	acted(taskId,u), collect(taskId,u,prop,get) >>> (
// 		r = apply(get),
// 		v = r !== undefined ? r : 'null',
// 		value(taskId,u,prop,v)
// 		);


// 	let handle = (f,v) => f(v);
// 	value(taskId,u,n,v), failOn(taskId,n,f,mess,clazz) | handle(f,v) >>> notification(mess,clazz), notvalid(taskId,u,n);

// 	- acted(t,u) >>> check(t,u); 

// 	- check(t,u), - notvalid(t,u,_) >>> cleanup(t,u); 
// 	- check(t,u) >>> cleanup(t,u), completed(t,u);

// 	cleanup(t,u), - notvalid(t,u,_);
// 	- cleanup(t,u);

// 	completed(taskId,u), - collect(taskId,u,prop,f);
// 	completed(taskId,u), - view(taskId,u,html) >>> remove(html);
// 	completed(taskId,u) >>> notifyCompleted(taskId);


// 	- notifyCompleted(taskId), label(taskId,L,_) >>> notification('Completed: ' + L,'notification taskId');


// 	- notification(t,clazz) >>> (
// 		div = addText('notificationArea',t,clazz),
// 		window.setTimeout(function() { module.clearNotification(div); chr.resolve(); }, 5000)
// 	);
 

// 	- clearNotification(html) >>> remove(html);
...
// };
	...
	let testinput = [path,pathVar,pathN,min,fib,xor,hamming,gcd,primes,sqrt,merge,comb,parser,parserdirect,parserexpl,select,test,playground];
 	...
 	let TestCHR = {};
    TestCHR.VERSION = VERSION;version;
	TestCHR.timescript = timescript;timescript;


	TestCHR.iterate = iterate;
	TestCHR.appendDivText = appendDivText;

	TestCHR.appendDivText = appendDivText;
	TestCHR.newline = newline;
	TestCHR.eqArray = eqArray;
	TestCHR.between = between;
	TestCHR.randomN = randomN;
	TestCHR.showArray = showArray;

	TestCHR.wrap = wrap;
	TestCHR.wrapExpr = wrapExpr;
	TestCHR.wrapModule = wrapModule;
